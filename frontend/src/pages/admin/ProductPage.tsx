import PaginationWrapper from "@/components/common/PaginationWrapper";
import { ErrorMessage } from "@/components/ui/ErrorMessage";
import { LoadingScreen } from "@/components/ui/LoadingScreen";
import { ProductsHeader } from "@/components/admin/poduct/ProductHeader"; 
import { ProductsTable } from "@/components/admin/poduct/ProductTable"; 
import { useGetAllProducts } from "@/hooks/admin/useAdminProducts";
import type { ProductParams } from "@/types/common/product.types";
import { useState } from "react";
import { useSearchParams } from "react-router-dom";

const ProductPage = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const [search, setSearch] = useState<string>(searchParams.get("name") || "");

  const queryParams: ProductParams = Object.fromEntries(searchParams.entries());
  queryParams.page = queryParams.page ? Number(queryParams.page) : 1;
  queryParams.limit = queryParams.limit ? Number(queryParams.limit) : 10;

  const { data, isLoading, isError, refetch } = useGetAllProducts(queryParams);
  const products = data?.data.products || [];
  const totalProducts = data?.data.total || 0;
  const totalPages = data?.data.totalPages || 0;

  const applyFilters = (key: string, value: string) => {
    const updated = new URLSearchParams(searchParams);
    value ? updated.set(key, value) : updated.delete(key);
    updated.set("page", "1");
    setSearchParams(updated);
  };

  const clearAllFilters = () => {
    const updated = new URLSearchParams();
    updated.set("page", "1");
    setSearchParams(updated);
  };

  const goToPage = (page: number) => {
    const updated = new URLSearchParams(searchParams);
    updated.set("page", page.toString());
    setSearchParams(updated);
  };

  if (isLoading) {
    return <LoadingScreen title="Loading products" subtitle="Fetching products list" />;
  }

  if (isError) {
    return <ErrorMessage title="Failed to load products" refetch={refetch} />;
  }

  return (
    <div className="min-h-screen bg-gray-50 py-4 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        <ProductsHeader
          search={search}
          setSearch={setSearch}
          totalProducts={totalProducts}
          applyFilters={applyFilters}
          clearAllFilters={clearAllFilters}
        />

        <div className="mb-4">
          <ProductsTable products={products} />
        </div>

        {totalPages > 1 && (
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <PaginationWrapper totalPages={totalPages} currentPage={queryParams.page} goToPage={goToPage} />
          </div>
        )}
      </div>
    </div>
  );
};

export default ProductPage;
